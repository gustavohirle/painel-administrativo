...
const esquema = z.object({
  ...
  quantidade: z.coerce
    ...
    .int("Use um número inteiro de unidades")
...
export async function registrarContagem(
  ...
  if (data.getTime() > Date.now()) {
    return { ok: false, mensagem: "A data da contagem não pode estar no futuro." };
  }
...
export async function removerContagem(
  ...
  if (!id) return { ok: false, mensagem: "Contagem não encontrada." };
